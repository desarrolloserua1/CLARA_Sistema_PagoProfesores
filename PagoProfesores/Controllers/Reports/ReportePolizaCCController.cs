...
{
    public class ReportePolizaCCController : Controller
    {
        private SessionDB sesion;
        private List<Factory.Privileges> Privileges;
        private database db;

        public ReportePolizaCCController()
        {
            db = new database();
            string[] scripts = { "js/Reports/ReportePolizaCC/ReportePolizaCC.js", "plugins/jquery-table2excel/jquery.table2excel.js" };
        ...
        public ActionResult Start()
        {
            ...
            ViewBag.Main = view.createMenu("Reportes", "Póliza contable por CC", sesion);
            ...
            return View(Factory.View.Access + "Reports/ReportePolizaCC/Start.cshtml");
        }
        ...
        public string CreateDateMes(string mes, string anio, string sede)
        {
            ...
            else
            {
                ...
                sb.Append("<thead><tr><td></td><td></td><td></td><td></td><th><b>" + fullMonthName.ToUpper() + anio + " </b></th><td></td><td></td><td></td><td></td><td></td><td></td><td></td><td></td></tr>");
        ...
        public string CreateDataTableHTML(int show = 25, int pg = 1, string search = "", string orderby = "", string sort = "", string filterMes = "", string filterAnio = "", string sede = "", string tipodocente = "", string pagosdeposito = "")
        {
            if (sesion == null) { sesion = SessionDB.start(Request, Response, false, db); }
            ...
            if (!filterMes.Equals(""))
            {
                ...
                string fullMonthName;
                string condicionsql = "";
                string condicionsql2 = "";
                string sqlselect = "SELECT * FROM QReportePolizaCC";
                string sql2 = "SELECT * FROM QNominaMesAnioSumCC";
                string sql;
                ...
                if ( (tipodocente != "" && tipodocente != "null") && (tipodocente != "T") )
                    condicionsql2 = " AND CVE_TIPOFACTURA = '" + tipodocente + "'";
                
                if (pagosdeposito != "" && pagosdeposito != "null")
                {
                    if (pagosdeposito == "PD")//PD pagos depositados o pagados(con fecha de deposito)
                    {
                        sql2 = "SELECT * FROM QNominaMesAnioSumCCDeposito";
                        sqlselect = "SELECT * FROM QReportePolizaCCDeposito";                       
                        //condicionsql2 = " AND (FECHADEPOSITO IS NOT NULL AND FECHADEPOSITO <> '' )";
                    }
                    else if (pagosdeposito == "PP")//PP pagos pendientes o no depositados (sin fecha de deposito)
                    {
                        sql2 = "SELECT * FROM QNominaMesAnioSumCCSINDeposito";
                        sqlselect = "SELECT * FROM QReportePolizaCCSINDeposito";                      
                        //condicionsql2 = " AND (FECHADEPOSITO IS NULL OR FECHADEPOSITO = '' )";
                    }               
                    else {
                        sql2 = "SELECT * FROM QNominaMesAnioSumCC";
                        sqlselect = "SELECT * FROM QReportePolizaCC";                       
                    }//T todos 
                }
                
                sql2 = sql2 + condicionsql + condicionsql2;
                ResultSet res2 = db.getTable(sql2);

                fullMonthName = new DateTime(2015, Int32.Parse(filterMes), 1).ToString("MMMM", CultureInfo.CreateSpecificCulture("es"));

                sb.Append("<table id=\"t1\" border=1>");
                sb.Append("<thead><tr><td></td><td></td><td></td><td></td><th><b>" + fullMonthName.ToUpper() + " " + filterAnio + "</b></th><td></td><td></td><td></td><td></td></tr>");//<td></td><td></td><td></td><td></td>
                sb.Append("<tr><th>Tipo de Transferencia</th>")
                    .Append("<th>Tipo de Pago</th>")
                    .Append("<th>Número de Cuenta</th>")
                    .Append("<th>Descripción CC</th>")
                    .Append("<th>Suma de Importe</th>")
                    .Append("<th>Suma de ImpIVA</th>")
                    .Append("<th>Suma de ImpIVARet</th>")
                    .Append("<th>Suma de ImpISRRet</th>")
                ...
                while (res2.Next())
                {
                    sql = sqlselect + " " + condicionsql + " and tipo_transferencia='" + res2.Get("tipo_transferencia") + "' and tipo_pago='" + res2.Get("tipo_pago") + "'";
                    ResultSet res = db.getTable(sql);
                    while (res.Next())
                ...
                while (res2.Next())
                {
                    ...
                    while (res.Next())
                    {
                        ...
                        sb.Append("</td>");
                        sb.Append("<td>");
                        sb.Append(res.Get("CENTRODECOSTOS"));//
                        sb.Append("</td>");
                        sb.Append("<td style='mso-number-format:\\0022$\\0022\\#\\,\\#\\#0\\.00'>");
                        decimal s0 = decimal.Parse(res.Get("importe"));
