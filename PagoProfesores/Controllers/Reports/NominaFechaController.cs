...
{
    public class NominaFechaController : Controller
    {
        private SessionDB sesion;
        private List<Factory.Privileges> Privileges;
        private database db;

        public NominaFechaController()
        {
            db = new database();
            string[] scripts = { "js/Reports/NominaFecha/NominaFecha.js", "plugins/jquery-table2excel/jquery.table2excel.js" };
            Scripts.SCRIPTS = scripts;

            Privileges = new List<Factory.Privileges> {
                 new Factory.Privileges { Permiso = 10146,  Element = "Controller" }, //PERMISO ACESSO AL REPORTE DE CALENDARIO DE PAGOS
                 new Factory.Privileges { Permiso = 10147,  Element = "formbtnConsultar" }, //PERMISO CONSULTAR CALENDARIO DE PAGOS
            };
        }
        ...
        public ActionResult Start()
        {
            ...
            ViewBag.Main = view.createMenu("Reportes", "Nomina Fecha", sesion);
            ...
            Log.write(this, "Start", LOG.CONSULTA, "Ingresa a pantalla 'Nomina por Fecha' ", sesion);

            return View(Factory.View.Access + "Reports/NominaFecha/Start.cshtml");
        }
        ...
        public string CreateDateMes(string mes,string anio, string sede)
        {
            ...
            else
            {
                ...
                while (res2.Next())
                {
                    ...
                    sb.Append(s.ToString("N2", CultureInfo.CreateSpecificCulture("es-MX")));
                    sb.Append("</td>");
                    sb.Append("<td style='mso-number-format:\\0022$\\0022\\#\\,\\#\\#0\\.00'>");
        ...
        public string CreateDataTableHTML(int show = 25, int pg = 1, string search = "", string orderby = "", string sort = "", string filterFecha = "",string sede = "")
        {
            if (sesion == null) { sesion = SessionDB.start(Request, Response, false, db); }
            ...
            string fullMonthName;
            string condicionsql = "";
            string sql2 = "SELECT * FROM QNominaFechaSum ";
            string sql;
            condicionsql = "where convert(varchar(12),fecha_de_pago,103)='" + filterFecha + "'" + " and sede='" + sede + "'";

            sql2 = sql2 + condicionsql;
                ResultSet res2 = db.getTable(sql2);
            if (!res2.HasRows)
            {
                sb.Append("<div class=\"jumbotron m-b-0 text-center\" style=\"width: 100 %; \"><h1>No existen registros para la fecha " + filterFecha + "</h1></div>");
            }
            else
            {
                sb.Append("<table id=\"t1\" border=1>");
                sb.Append("<thead><tr><td></td><td></td><td></td><td></td><th><b>" + filterFecha + "</b></th><td></td><td></td><td></td><td></td><td></td></tr>");
                sb.Append("<tr><th>Tipo de Transferencia</th> <th>Tipo de Pago</th><th>Fecha de Pago</th><th>ID empleado</th><th>Empleado</th><th>Suma de Importe</th><th>Suma de ImpIVA</th><th>Suma de ImpIVARet</th><th>Suma de ImpISRRet</th><th>Suma Bancos</th></tr> </thead>");
                ...
                while (res2.Next())
                {
                    ...
                    while (res.Next())
                    {
                        ...
                        sb.Append(s0.ToString("C", CultureInfo.CreateSpecificCulture("es-US")));
                        sb.Append("</td>");
                        sb.Append("<td style='mso-number-format:\\0022$\\0022\\#\\,\\#\\#0\\.00'>");
                        ...
                        sb.Append("</td>");
                        sb.Append("<td style='mso-number-format:\\0022$\\0022\\#\\,\\#\\#0\\.00'>");
                        double s3 = double.Parse(res.Get("importe_isr_retenido"));
                        sb.Append(s3.ToString("C", CultureInfo.CreateSpecificCulture("es-MX")));
                        sb.Append("</td>");
                        sb.Append("<td style='mso-number-format:\\0022$\\0022\\#\\,\\#\\#0\\.00'>");
        ...
        public void ExportExcel()
        {
            ...
            try
            {
                ...
                using (ExcelPackage pck = new ExcelPackage())
                {
                    ...
                    ExcelWorksheet ws = pck.Workbook.Worksheets.Add("NominaFecha");
                    ...
                    Response.AddHeader("content-disposition", "attachment;  filename=NominaFecha.xlsx");
